...
import {
  Constants,
  arrayUnique,
  targetPositionHasChanged,
  popoverInfosAreEqual,
} from "./util"
...
class Popover extends React.Component<PopoverProps, PopoverState> {
  ...
  public componentDidUpdate(prevProps: PopoverProps) {
    ...
    const hasNewDestination =
      prevProps.contentDestination !== this.props.contentDestination
  ...
  public render() {
    ...
    if (
      (this.props.isOpen || isTransitioningToClosed) &&
      this.popoverDiv &&
      popoverInfo
    ) {
      ...
      (violation, rect) => {
        ...
        if (
          ...
          const {
            top: nudgedTop,
            left: nudgedLeft,
          } = this.getNudgedPopoverPosition(rect)
          ...
          this.renderWithPosition(
            ...
            () => {
              ...
              if (
                ...
                !this.state.isTransitioningToClosed
              ) {
                this.popoverDiv.style.opacity = "1"
              }
            },
  ...
  private renderWithPosition(
    ...
    callback?: (
      ...
      resultingRect: Partial<ClientRect>,
    ) => void,
  ) {
    ...
    if (!popoverInfosAreEqual(this.state.popoverInfo, popoverInfo)) {
      window.clearTimeout(this.removePopoverTimeout)
      this.setState(
        { popoverInfo, isTransitioningToClosed: false, internalisOpen: true },
        () => {
          if (this.willUnmount) {
            return
          }
          ...
          const { top, left } = this.getLocationForPosition(
            position,
            targetRect,
            popoverRect,
          )
          ...
          callback(
            (position === "top" && top < padding) ||
              ...
              (position === "right" &&
                left + popoverRect.width > window.innerWidth - padding) ||
              (position === "bottom" &&
                top + popoverRect.height > window.innerHeight - padding),
    ...
    if (position && typeof position !== "string") {
      if (
        Constants.DEFAULT_POSITIONS.every(
          (defaultPosition) =>
            position.find((p) => p === defaultPosition) !== undefined,
        )
      ) {
    ...
    if (containerStyle) {
      Object.keys(containerStyle).forEach(
        (key) =>
          (container.style[
            key as any
          ] = (containerStyle as CSSStyleDeclaration)[key as any]),
      )
    }
